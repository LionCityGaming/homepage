...
const IFRAME_CONFIG = [
  {
    selector: ".home-assistant, #hawidget", // Added #hawidget
  ...
  {
    selector: ".chelsea, #upcomingtv", // Added #upcomingtv
    src: "{{HOMEPAGE_VAR_CHELSEA_WIDGET_URL}}",
  },
  {
    selector: ".epl",
    src: "{{HOMEPAGE_VAR_EPL_STANDINGS_WIDGET_UR}}",
  },
];
const TAB_MAPPING = {
...
async function refreshService(card) {
  ...
  try {
    ...
    const response = await (apiEndpoint.includes("CrowdSec") &&
    apiEndpoint.includes("alerts")
      ? handleCrowdSecAlerts(card, apiEndpoint)
      : fetch(apiEndpoint).then((r) =>
...
function initializeEverything() {
  ...
  if (
    ...
    domCache.initCache();
    initializeTabFocus();
    preloadIframes(); // Now with retry mechanism
    preloadAllTabs();
...
const resizeObserver = new ResizeObserver((entries) => {
  entries.forEach((entry) => {
    ...
    if (container.matches("#hawidget, #upcomingtv")) {
      const iframe = container.querySelector("iframe");
      if (iframe) {
...
document.querySelectorAll("#hawidget, #upcomingtv").forEach((container) => {
